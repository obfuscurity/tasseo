...
TasseoDashboard.prototype = {
  ...
  refreshData: function(immediately) {
    this.datasource.refresh(this.metrics, this.period, function(metric) {
      if (immediately) {
        metric.redraw()
      }
    });
...
function TasseoMetric(element, metricSpec, dashboardOptions) {
  ...
  this.alias = metricSpec.alias || metricSpec.target;
  this.description = metricSpec.description;
...
TasseoMetric.prototype = {
  ...
  update: function(newData) {
    _.each(newData, function(record, idx) {
      if (!_.isUndefined(record)) {
        this.datum[idx] = record
      }
    }, this)

    // check our thresholds and update color
    var lastValue = _.last(this.datum).y
    var warning = this.metricSpec.warning;
    var critical = this.metricSpec.critical;
    if (critical > warning) {
  ...
  redraw: function() {
    ...
    if (!_.isUndefined(_.last(this.datum))) {
      ...
      if (!_.isUndefined(this.metricSpec.unit)) {
        element.find('.overlay-number').append('<span class="unit">' + this.metricSpec.unit + '</span>');
      }
    } else {
...
function TasseoGraphiteDatasource(url, auth, period, options) {
  this.baseUrl = url;
  this.auth = auth;
  this.period = period
...
TasseoGraphiteDatasource.prototype = {
  refresh: function(metrics, period, callback) {
    $.ajax({
      beforeSend: function(xhr) {
        if (this.auth && this.auth.length > 0) {
          var bytes = Crypto.charenc.Binary.stringToBytes(this.auth);
      ...
      if (d.length > 0) {
        _.each(d, function(datapoints, idx) {
          var metric = metrics[idx];
          metric.update(_.map(datapoints, function(datapoint) {
            return { x: datapoint[1], y: datapoint[0] }
          }))

          callback.call(metric)
        })
      }
    })
  },
  ...
  urlForMetrics: function(metrics, period) {
    var targets = _.map(metrics, function(metric) {
      if (this.options.padnulls) {
        'target=keepLastValue(' + encodeURI(metric.metricSpec.target) + ')'
      } else {
        'target=' + encodeURI(metric.metricSpec.target)
      }
    }).join("&");

    var myUrl = this.url + '/render/?' + targets + '&from=-' + period + 'minutes&format=json';
    return myUrl;
  }
}
...
TasseoUi.prototype = {
  ...
  attach: function() {
    ...
    $('.toolbar ul li.timepanel').on('click', 'a.range', function() {
      ...
      self.dashboard.refreshData(true);
    });
