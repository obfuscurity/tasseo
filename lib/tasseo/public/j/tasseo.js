...
// various graph options passed through Rickshaw to D3
var interpolation = (typeof interpolation == 'undefined') ? 'step-after' : interpolation;
var renderer = (typeof renderer == 'undefined') ? 'area' : renderer;
var stroke = (typeof stroke == 'undefined') ? true : stroke;

// allow custom colors
var criticalColor = (typeof criticalColor == 'undefined') ? '#d59295' : criticalColor;
var warningColor = (typeof warningColor == 'undefined') ? '#f5cb56' : warningColor;
var normalColor = (typeof normalColor == 'undefined') ? '#afdab1' : normalColor;
    ...
    for (var i=0; i<graphs.length; i++) {
      ...
      if (critical > warning) {
        if (lastValue >= critical) {
          graphs[i].series[0].color = criticalColor;
        } else if (lastValue >= warning) {
          graphs[i].series[0].color = warningColor;
        } else {
          graphs[i].series[0].color = normalColor;
        }
      } else {
        if (lastValue <= critical) {
          graphs[i].series[0].color = criticalColor;
...
// option to hide our title
var title = (typeof title == 'undefined') ? true : title;
if (!title) { $('div.title span').css('visibility', 'hidden'); }

// option to hide our toolbar
var toolbar = (typeof toolbar == 'undefined') ? true : toolbar;
if (!toolbar) { $('div.toolbar').css('visibility', 'hidden'); }
