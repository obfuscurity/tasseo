...
var Tasseo = {
  ...
  refreshData: function (immediately) {
    this.refresh(function(metric, values) {
  ...
  buildContainers: function() {
    for (var metric in metrics) {
      if (metrics[metric].target === false) {
        ...
        var link_open = 'link' in metrics[metric] ? '<a href="' + metrics[metric].link + '" target="_new">' : '';
...
var Graphite = {
  padnulls: (typeof padnulls == 'undefined') ? true : padnulls,

  start: function() {
    this.constructUrl(this.period);
  },

  reset: function(period) {
    this.constructUrl(period);
  },

  constructUrl: function(period) {
    var targets = [];
    for (var metric in this.realMetrics) {
      if (this.padnulls === true) {
        targets.push('target=keepLastValue(' + encodeURI(this.realMetrics[metric].target) + ')');
      } else {
        targets.push('target=' + encodeURI(this.realMetrics[metric].target));
      }
    }
    this.url = this.options.url + '/render/?' + targets.join("&") + '&from=-' + period + 'minutes&format=json';
  },
    ...
    $.ajax({
      ...
      if (d.length > 0) {
        for (var i = 0; i < d.length; i++) {
          var metric = d[i].target.replace(/^[a-zA-Z]+\((.+)\)$/, '$1');
          myDatum = [];
          for (var j = 0; j < d[i].datapoints.length; j++) {
            myDatum[j] = { x: d[i].datapoints[j][1], y: d[i].datapoints[j][0] };
          }
          cb.call(Graphite, metric, myDatum);
        } 
      }
    });
  },
...
$.extend(Graphite, Tasseo);

var tasseo = Tasseo.backend(TasseoOptions);
...
$('.toolbar ul li.timepanel').on('click', 'a.play', function() {
  ...
  $('.toolbar ul li.timepanel a.play').text(tasseo.period + 'min');
  tasseo.refreshData("now");
